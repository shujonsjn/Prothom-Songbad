          ...
          mapDiv.innerHTML = '';
            ...
            const map = new jsVectorMap({
              ...
              onRegionTooltipShow(evt, tooltip, code){
                ...
                if(item){
                  const html = '<div style="font-size:13px;font-weight:500;padding:2px 0;">' + esc(item.name) + ' <span style="opacity:.7;font-size:11px;">(' + esc(item.country) + ')</span></div><div style="font-size:13px;font-weight:600;color:#6750a4;">' + item.count + ' visits <span style="font-weight:400;font-size:11px;color:#999;">(' + item.pct + '%)</span></div>';
                  setTimeout(() => { tooltip.innerHTML = html; }, 0);
                }
              },
